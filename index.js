...
client.ayarlar = {
  oynuyor: "Bu bot şuan geliştirilmektedir.",
  official_sahip: "624346501010685969",
  sahip: "624346501010685969",

  yardimcilar: [""],
  isim: "Uzay",
  botD:
    "https://discord.com/oauth2/authorize?client_id=727596656136290436&scope=bot&permissions=8",
  webS: "https://blacklive.glitch.me",
  web: "https://blacklive.glitch.me",
  site: "https://blacklive.glitch.me",
  dblO: "",
  dbl: "",
  dbltoken: "",
  webpanel: "https://blacklive.glitch.me",
  versiyon: "2.0",
  prefix: "!!",
  renk: "RANDOM",
  version: "1.0.0"
  ...
  if (user) {
    ...
    if (offlineetiket === 1) {
      const paras = new Discord.RichEmbed()
        ...
        .setAuthor(
          `${ben}, ${msg.guild.name} Sunucusunda ${msg.channel.name} kanalında sizi etiketledi.`,
...
const youtube = new YouTube("AIzaSyCBbym789KsRSdejVK9hew1z6el9j06ysk");
const queue = new Map();
      ...
      if (msg.content.length > 64) {
        var embed = new Discord.RichEmbed()
          ...
          .setDescription(
            `${client.emojis.get(
              "672906896893673556"
            )} Hizzaya Geçin! Burada Bir Gold Üye Belirdi! <@${msg.author.id}>`
          )
          .setColor("BLUE");
  ...
  if (yashinubanlimit) {
    if (entry.executor.id !== guild.owner.user.id) {
      ...
      client.channels
        .get(log)
        .send(
          `\`${user.id}\` - \`${user.tag}\` kişisi ${
            entry.executor
          } tarafından **${
            entry.reason ? entry.reason : "girilmedi"
          }** nedeni ile yasaklandı! \n${
            entry.executor
          } Banları: ${yashinukullanıcıban}`
        );
  ...
  if (yashinukanallimit) {
    if (entry.executor.id !== guild.owner.user.id) {
      ...
      client.channels
        .get(log)
        .send(
          `\`${channel.name}\` adlı kanal ${entry.executor} tarafından silindi!`
        );
      ...
      if (yashinukullanıcılimit >= yashinukanallimit) {
        try {
          client.channels
            .get(log)
            .send(
              `Sunucundan bir yetkili kanal limitine ulaştı ve sunucudan atıldı ! İşte bilgileri => \n\n\`Kullanıcı:\`  ${
                entry.executor
              } | ${
                entry.executor.id
              } \n\`Discord'a ve Sunucuya Katılım Tarihi:\` \n• **Discord:** ${moment(
...
client.login("NzI3NTk2NjU2MTM2MjkwNDM2.XvyX9w.blHvv9qDnhcv5JK6-YotI2HBKAc");
