...
${footer}`
  ...
  let cats = arr
    ...
    .map(
      k =>
        `• \`${prefix}${dil.yardımm} ${k} \`-> ${k.charAt(0).toUpperCase() +
          k.slice(1)} ${dil.showw}`
    )
    .join("\n");
...
`
    ...
    if (!arr.includes(arg))
      return msg.channel.send(`**${arg}** ${dil.yardım.hata2}`);
    ...
    const e = new Discord.RichEmbed()
      ...
      .setDescription(
        list
          .map(
            k =>
              `**${prefix + k.help.name}**${" ".repeat(
                longest - k.help.name.length
              )}: ${k.help.description}`
          )
          .join("\n") + `\n\n${footer}`
      );
