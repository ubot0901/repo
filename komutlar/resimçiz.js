...
class Drawing {
  constructor(msg, size, fg, bg) {
    ...
    this.initPixels();
    this.c = new Canvas(this.realsize, this.realsize)
      .setColor(this.bcolor)
      .addRect(0, 0, this.realsize, this.realsize);
    this.drawCanvas();

    msg.edit(
      "Kalemi hareket ettirmek için reaksiyonları kullanın:  n✅ Çizimi Durdur |    Kalem Açık | Off Kalem Kapalı"
    );
    this.reactArrows(0);
    ...
    reaction.remove(reaction.users.last()).catch(e => {
      if (e.code === 50013)
        reaction.message.channel.send(
          "Düzgün çalışması için 'Mesajları Yönet' iznine ihtiyacım var!"
        );
    });
    this.drawCanvas();
  }
  ...
  movePen(dir) {
    ...
    if (
      (this.penx > 0 || xactions[dir] === 1) &&
      (this.penx < this.size || xactions[dir] === -1)
    )
      this.penx += xactions[dir];
    if (
      (this.peny > 0 || yactions[dir] === 1) &&
      (this.peny < this.size || yactions[dir] === -1)
    )
      this.peny += yactions[dir];
  }
  ...
  setCanvasPixel(x, y, color) {
    this.c
      .setColor(color)
      .addRect(x * zoomfactor, y * zoomfactor, zoomfactor, zoomfactor);
  }
    ...
    for (let x = 0; x < this.size; x++) {
      for (let y = 0; y < this.size; y++) {
        this.setCanvasPixel(
          x,
          y,
          this.pixels[x + y * this.size] ? this.fcolor : this.bcolor
        );
      }
    }
    if (!end) {
      this.setCanvasPixel(
        this.penx,
        this.peny,
        this.pixels[this.penx + this.peny * this.size] ? "#5A0000" : "red"
      );
      this.renewTimeout();
    }
    this.sendCanvas();
  ...
  async sendCanvas() {
    ...
    this.msg.channel
      .send(`Canvas: ${this.size}px`, {
        files: [this.c.toBuffer()]
      })
      .then(msg => {
        this.canvasmsg = msg;
      });
